...
class Product extends Model{
	...
	*/
	...
	function getProductsByCategory($category_id, $sort = 'new', $filters = [],$get_total_records = null)
	{
		if($category_id)
		{
			...
			$query = DB::table('products as p')->select('p.*','c.display_order')->leftJoin('product_to_category as c','p.id','=','c.product_id')->leftJoin('product_to_color as clr','p.id','=','clr.product_id')->where('p.status','1')->whereIN('c.category_id',$categoryList);
		}
		else
		{
			$query = DB::table('products as p')->select('p.*')->leftJoin('product_to_color as clr','p.id','=','clr.product_id')->where('p.status','1');
		}
		...
		if(sizeof($filters) > 0)
		{
			...
			if(isset($filters['price_from']))
				$query = $query->where('p.sale_price','>=',$filters['price_from']);

			if(isset($filters['price_to']))
				$query = $query->where('p.sale_price','<=',$filters['price_to']);
		...
		if($sort == 'priceAsc'){
			$query = $query->orderBy('p.sale_price', 'ASC');
		}else if($sort == 'priceDesc'){
			$query = $query->orderBy('p.sale_price', 'DESC');
		}else if($sort == 'a-z'){
	...
	*/
	...
	public function getBestsellers($limit = 20){
		$query = DB::table('products as p');
		$query->select('p.*', DB::raw('(SELECT SUM(otp.quantity) as total from order_to_product as otp WHERE otp.product_id = p.id) as totalItems'));
		$query->leftJoin('product_to_color as clr','p.id','=','clr.product_id');
	...
	*/
	...
	{
		$query = DB::table('products as p')->select('p.*','pc.display_order')->leftJoin('product_to_category as pc','p.id','=','pc.product_id')->leftJoin('categories as c','pc.category_id','=','c.id')->leftJoin('product_to_color as clr','p.id','=','clr.product_id')->where('p.status','1');
	...
	*/
	...
	function getAllBrandFilters($filters)
	{
		...
		$query = DB::table('products as p')->select('p.brand_id','b.title as brand_name',DB::raw('count(distinct p.id) as total_products'))->leftJoin('brands as b','p.brand_id','=','b.id')->leftJoin('product_to_category as pc','p.id','=','pc.product_id')->leftJoin('categories as c','pc.category_id','=','c.id')->leftJoin('product_to_color as clr','p.id','=','clr.product_id')->where('p.status','1')->where('p.brand_id','<>',0);
