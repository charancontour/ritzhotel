...
class ProductsController extends Controller {
	...
	*/
				...
				if (($handle = fopen("public/uploads/products.csv", "r")) !== FALSE) {
					...
					while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
						...
						$products[] = array(
							...
							'product_name'                  => $data[1], // products table
							'product_code'                  => $data[2], // products table
	...
	function editProduct($product_id)
	{
		...
		if(Request::isMethod('post'))
		{
			$messages = ['large_image.required' => 'Max file size should be less than 2MB.',
						'thumbnail_image_1.required' => 'Max file size should be less than 2MB.',
						'thumbnail_image_2.required' => 'Max file size should be less than 2MB.',
						'max' => 'Max file size should be less than 2MB.'
						];

			$validation_rules = array('product_name' => 'required',
										'product_code' => 'required',
										'categories' => 'required',
										'sale_price' => 'required');
		  ...
		  if ($validator->fails()) {
			...
			else
			{
				...
				if(Input::get('quantity_in_stock') > 0){
					...
					if($users){
						$ids = array();
						$messageBody = 'Hello,<br />We\'ll like to let you know that product ' . Input::get('product_name') . ' ' . Input::get('product_code') . ' is now available in our site. We have ' . Input::get('quantity_in_stock') . ' quantity in our stock.';

						foreach($users as $users){
							$messageData = [
								'fromEmail' 			=> 'tbmshop@tbm.com',
								'fromName' 				=> 'TBM Shop',
								'toEmail' 				=> $users->email,
								'toName' 				=> $users->name,
								'subject'				=> 'Product ' . Input::get('product_name') . ' ' . Input::get('product_code') . ' is now available!!!'
							];
