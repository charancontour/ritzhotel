	...
	*/
	...
	function editProduct($product_id)
	{
		...
		if(Request::isMethod('post'))
		{
			...
			$validation_rules = array('type' => 'required',
										'room_code' => 'required',
										'categories' => 'required',
										'sale_price' => 'required');
		  ...
		  if ($validator->fails()) {
			...
			else
			{
				...
				if(Input::get('quantity_in_stock') > 0){
					...
					if($users){
						$ids = array();
						$messageBody = 'Hello,<br />We\'ll like to let you know that product ' . Input::get('type') . ' ' . Input::get('room_code') . ' is now available in our site. We have ' . Input::get('quantity_in_stock') . ' quantity in our stock.';
						...
						foreach($users as $users){
							$messageData = [
								...
								'toEmail' 				=> $users->email,
								'toName' 				=> $users->name,
								'subject'				=> 'Product ' . Input::get('type') . ' ' . Input::get('room_code') . ' is now available!!!'
							];
	...
	function listProducts()
	{
		...
		// get last updated
		$this->data['last_modified'] = DB::table('products')->orderBy('last_modified','desc')->pluck('last_modified')[0];
	...
	{
            $desc = strip_tags(Request::input('content'));
