	...
	*/
	...
	public function resetmail(Request $request)
	{
		...
		else
		{
			...
			$to = $formData->email;
			$to_name = $formData->first_name;
			$from = 'passwordreset@ritzgardenhotel.com';
			$from_name = 'Password Reset';
			$subject = "Reset Password in Ritz Garden Hotel";

			$message = "Hello ".$formData->first_name."<br><br>";
			$message .= "Please <a href='http://". Request::getHttpHost() ."/passwordreset?email=".$to."&code=".$code."'>click here</a> to reset your password.<br/><br/>";
			$message .= "<img alt='logo' src='http://". Request::getHttpHost() ."/public/front/images/index/logo.png'>";
			$message .= "<br><br>Thank you.<br/>";
			$message .= "<br><br>Best Regards,<br/>";
			$message .= "Ritz Garden Hotel Online Registration Manager<br/>";
			$message .= "Ritz Garden Hotel<<br/>";
			$message .= "No. 86 & 88, Jalan Yang Kalsom, 30250 Ipoh Perak Darul Ridzuan, Malaysia.<br/>";
			$message .= "Tel: (05) 242-7777<br/>";
			$message .= "Fax: (603) 242-5845<br/>";
			$message .= "sales@ritzgardenhotel.com<br/>";
			$message .= "lily@ritzgardenhotel.com<br/>";
			...
			Mail::raw($message, function($message) use ($formData)
			{
				$message->subject("Reset Password in Ritz Garden Hotel");
				$message->to($formData->email)->getSwiftMessage()->getHeaders()->addTextHeader('Content-type', 'text/html');
			});
...
/************************************************/
	function passwordresetpost()
	{
		...
		if(Request::isMethod('post'))
		{
			...
			if ($validator->fails()) {
				$errors = $validator->errors()->all() ;
				return Redirect::to(Request::url()."/passwordreset?email=".$email."&code=".$code."" )->withInput()->with('error', 'Password length should be between 8-12 characters with combination of alphabet letters, digits & special characters (eg. *@$!+%~).')->with('errors', $errors);
			}else{
