        ...
        jQuery.ajax({
            ...
            .done(function(response){
                if(response != 0){
                    ...
                    jQuery('.total-table-title.text-danger').html('DISCOUNT: ('+Number((res.promo_code)).toFixed(2)+')');
                    var total = Number(jQuery('#ca-total').html().replace('RM ', '')).toFixed(2);
                    jQuery('.amount.text-danger').html('RM '+(Number(total)*(Number(res.discount)/100)).toFixed(2)+'');
...
?>
 ...
 html += '</td>';
 html += '<td class="item-total-col"><span class="item-price-special s-b"><span class="mp">RM '+ parseInt(parseInt(row.sale_price) * parseInt(GetNightsFromDates())) +'.</span><span class="sub-prices">00</span></span>';
